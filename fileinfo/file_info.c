...
int get_info(char* csFileName)
{
	...
	{
		//Links. output "S,Name,Size,Mask,LMT,link destination
		char 	link_dest[260];
		char    *file_string = (csFileName[0] == '/' ? csFileName+1 : csFileName);
		char    cmd_line[260];

		sprintf(cmd_line, "readlink \"%s\" > %s", file_string, "/tmp/.filee");
  	        system(cmd_line);
		
		FILE *fp = fopen("/tmp/.filee","r");
		fgets(link_dest, 260, fp);
		if (link_dest[strlen(link_dest)-1] == '\n') 
			link_dest[strlen(link_dest)-1] = '\0';

		printf("S,%s,%d,%04o,%d,%d,%d,%s\n", pOutName, iTestSize,
